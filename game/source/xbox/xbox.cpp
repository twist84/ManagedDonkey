...
struct XShowKeyboardUI_struct
{
	...
	void dialog_initialize_handler(HWND dialog_handle)
	{
		...
		SetWindowText(GetDlgItem(dialog_handle, IDTEXT), default_text);
		SetFocus(GetDlgItem(dialog_handle, IDTEXT));
	}
	...
	void dialog_command_handler(HWND dialog_handle, long command)
	{
		...
		{
		case IDOK:
			SendMessage(GetDlgItem(dialog_handle, IDTEXT), WM_GETTEXT, (WPARAM)maximum_character_count, (LPARAM)result_text);
...
long XShowKeyboardUI(long controller_index, unsigned long character_flags, wchar_t const* default_text, wchar_t const* title_text, wchar_t const* description_text, wchar_t* result_text, unsigned long maximum_character_count, void* platform_handle)
{
	...
	DialogBoxParam((HINSTANCE)platform_handle, MAKEINTRESOURCE(IDD_DIALOG1), g_game_window_handle, &XShowKeyboardUI_proc, (LPARAM)&params);
