...
public:
	...
	c_bitstream(uint8* data, int32 data_length) :
	...
	void write_integer(char const* name, uint32 value, int32 size_in_bits);
	...
	uint64 read_qword(char const* name, int32 size_in_bits);
	void write_qword(char const* name, uint64 value, int32 size_in_bits);
...
private:
	void encode_qword_to_memory(uint64 value, int32 size_in_bits);
...
public:
	...
	uint8 const* get_data(int32* data_length) const;
	void push_position();
	void pop_position(bool pop);

private:
	uint64 read_accumulator_from_memory(int32 a1);

public:
	bool read_bool(char const* name);
	void read_bits_internal(void* data, int32 size_in_bits);
	uint32 read_integer(char const* name, int32 size_in_bits);
...
public:
	bool reading() const;
	void set_data(uint8* data, int32 data_length);
	void skip(int32 bits_to_skip);
	bool would_overflow(int32 size_in_bits) const;
	void write_accumulator_to_memory(uint64 value, int32 size_in_bits);
...
protected:
	...
	uint32 __unknown34; // int32 m_potential_bit_position?
