...
extern uint8 data_encode_array(data_encoding_state* state, long element_count_size, void* source_array, long element_count, byte_swap_definition* bs_definition);
extern uint8 data_encode_integer(data_encoding_state* state, long value, long maximum_value);
extern uint8 data_encode_memory(data_encoding_state* state, void const* buffer, short count, long code);
extern void data_encode_new(data_encoding_state* state, void* buffer, long buffer_size);
extern uint8 data_encode_string(data_encoding_state* state, char* source_string, short maximum_string_length);
extern uint8 data_encode_structures(data_encoding_state* state, void const* source_structures, short structure_count, byte_swap_definition* bs_definition);
