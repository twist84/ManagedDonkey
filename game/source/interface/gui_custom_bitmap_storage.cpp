...
void __thiscall c_gui_custom_bitmap_storage_item::initialize(long width, long height, bool use_compressed_format)
{
	...
	dword base_size = 0;
	dword mip_size = 0;
	dword resource_bytes = XGSetTextureHeader(m_bitmap.width, m_bitmap.height, 1, 4, d3d_format, 0, 0, -1, 0, texture_header, &base_size, &mip_size);
	if (resource_bytes)
	{
...
bool __thiscall c_gui_custom_bitmap_storage_item::load_from_buffer(char const* buffer, long buffer_size, void* buffer2, long buffer2_size, long a6)
{
	...
	if (d3d_texture == NULL)
	{
		generate_event(_event_error, "ui:custom_bitmaps: d3d_texture is null");
		return false;
	}
	...
	{
		generate_event(_event_error, "ui:custom_bitmaps: D3DXLoadSurfaceFromFile failed with error code 0x%08X", load_surface_result);
		return false;
	}
