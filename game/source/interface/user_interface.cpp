...
void __cdecl user_interface_scoreboard_update()
{
    ...
    if (user_interface_should_show_console_scoreboard(0))
    {
        ...
        c_gui_screen_scoreboard* screen_scoreboard = c_gui_screen_scoreboard::get_scoreboard_screen(k_any_controller);
        if (screen_scoreboard)
        {
            c_window_manager* window_manager = window_manager_get();
            if (window_manager->get_screen_above(_window_index4, screen_scoreboard))
    ...
    for (long i = 0; i < k_number_of_controllers; i++)
    {
        ...
        bool controller_signed_in = false;
        if (controller->is_signed_in_to_machine() || TEST_BIT(controller->m_flags, 3))
            controller_signed_in = true;
    
        if (!controller_signed_in && !user_interface_should_show_console_scoreboard(0))
