			...
			if (user_interface_squad_is_player_valid(session_player_index))
			{
				...
				if (player_data->host.team_index != -1)
				{
					...
					add_player_internal(
						...
						/* player_name          */ player_data->host.name,
						/* service_tag          */ player_data->host.appearance.service_tag,
				...
				if (player->configuration.host.team_index != -1)
				{
					...
					if (network_player_index != -1)
					{
						...
						add_player_internal(
							...
							/* player_name          */ player->configuration.host.name,
							/* service_tag          */ player->configuration.host.appearance.service_tag,
							/* base_color           */ base_color,
