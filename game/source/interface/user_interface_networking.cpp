...
void __cdecl user_interface_networking_clear_last_game_started()
{
	...
	MESSAGE_EVENT("networking:ui: clearning last game started flag");
...
void __cdecl user_interface_networking_notify_booted_from_session(e_network_session_type type, e_network_session_boot_reason boot_reason)
{
	...
	MESSAGE_EVENT("networking:ui: notified that we have been booted from a session [type %d / reason %d]", type, boot_reason);

	if (boot_reason == _network_session_boot_from_ui)
	{
		MESSAGE_EVENT("networking:ui: posting alert and taking us back to the pre-game lobby (from-ui)");
		user_interface_error_manager_get()->post_error(STRING_ID(gui_alert, booted_from_session), k_any_controller, false);
	}
	else
	{
		MESSAGE_EVENT("networking:ui: posting alert and taking us back to the pre-game lobby (in-game)");
...
bool __cdecl user_interface_reset_networking_to_pregame()
{
	...
	{
		ERROR_EVENT("ui:networking:user_interface_reset_networking_to_pregame: we are not the leader of a custom game");
		return false;
	}

	if (!network_life_cycle_set_pre_game_state())
	{
		ERROR_EVENT("ui:networking:user_interface_reset_networking_to_pregame: failed to return networking to pregame mode");
		return false;
	}
