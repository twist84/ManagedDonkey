...
bool __cdecl user_interface_available_squads_initialize(long controller_index, dword_flags available_squad_search_flags, char category)
{
	...
	g_available_squads_ready = network_search_start(controller_index, category != 0, 1 /* default is `25`, this causes an issue not having enough memory allocate */, available_squad_search_flags, allocation);
