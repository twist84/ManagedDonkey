...
void __cdecl game_dispose_from_old_structure_bsp(dword deactivating_structure_bsp_mask)
{
...
void __cdecl game_globals_initialize_for_new_map(game_options const* options)
{
	...
	if (game_globals->options.game_mode == _ui_game_mode_multiplayer || options->multiplayer_variant.get_game_engine_index())
	{
		...
		if (!game_engine_variant_validate(&game_globals->options.multiplayer_variant))
			WARNING_EVENT("variant validation failed, about to start playing a default variant");
	}
...
void __cdecl game_initialize_for_new_structure_bsp(dword activating_structure_bsp_mask)
{
