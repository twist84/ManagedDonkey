...
//.text:005504C0 ; long __cdecl game_engine_get_number_of_rounds()
...
void __cdecl game_engine_dump_variant_settings(char const* filename)
{
	if (game_is_multiplayer() && current_game_engine() && current_game_variant())
	{
		...
		if (file_reference_create_from_path(&file, filename, false))
		{
			...
			{
				generate_event(_event_error, "game_engine: failed to create file to write variant settings!");
			}
		}
	}
	else
	{
		generate_event(_event_error, "game_engine: this game cannot export game engine variant settings!");
	}
}
