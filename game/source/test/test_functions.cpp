...
void test_download_storage_file_update()
{
	...
	if (g_test_download_storage.request_cookie)
	{
		...
		case 1:
		{
			generate_event(_event_level_message, "test:download_storage_file: http request completed successfully.");
			g_test_download_storage.request_cookie = 0;
			v2 = false;
		}
		break;
		case 2:
		{
			generate_event(_event_level_message, "test:download_storage_file: http request failed, cancelling.");
			g_test_download_storage.request_cookie = 0;
			v2 = false;
		}
		break;
		case 3:
		{
			generate_event(_event_level_message, "test:download_storage_file: http request failed, retrying.");
			g_test_download_storage.request_cookie = 0;
			v2 = false;
		}
		break;
