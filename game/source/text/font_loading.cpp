...
e_async_completion __cdecl font_load_callback(s_async_task* task)
{
	...
	if (loading_state->file_open)
	{
		...
		if (file_read(&loading_state->file_reference, sizeof(s_font_package_file), false, &g_font_globals.package_file))
		{
			...
			if (font_package_file_header_validate(&g_font_globals.package_file.header))
			{
				...
				for (long font_index = 0; font_index < g_font_globals.package_file.header.font_count && !load_package_file_failed; ++font_index)
				{
					...
					{
						ERROR_EVENT("package file font #%d header offset %d (size %d) not valid in [%d, %d+%d]",
							font_index,
							font.header_offset,
							font.header_size,
					...
					else
					{
						...
						{
							ERROR_EVENT("package file font #%d header failed to validate",
								font_index);
				...
				if (!load_package_file_failed)
				{
					...
					MESSAGE_EVENT("loaded package file '%s' (%d fonts)",
			...
			{
				ERROR_EVENT("package header failed to validate");
			}
		...
		{
			ERROR_EVENT("fonts: couldn't read package file header (%d bytes)",
