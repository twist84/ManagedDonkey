...
static_assert(NUMBEROF(key_names) == k_key_code_count);
...
const char* button_names[] =
{
	...
	"lean_right",
	"flashlight",
	"machinima_lower_weapon_toggle",
	"machinima_camera_enable_toggle",
	"machinima_camera_control_toggle",
	"machinima_camera_debug_toggle",
	"general_chat",
	"team_chat",
	"voice_chat",
	"unknown42",
...
static_assert(NUMBEROF(button_names) == NUMBER_OF_KEYBOARD_CONTROL_BUTTONS);
...
void __cdecl input_abstraction_get_input_state(int32 controller_index, s_game_input_state** input_state)
{
	...
	*input_state = &input_abstraction_globals.input_states[controller_index];
	...
	s_gamepad_input_preferences* preferences = &input_abstraction_globals.preferences[controller_index];

	for (int32 control_button_index = 0; control_button_index < NUMBER_OF_KEYBOARD_CONTROL_BUTTONS; control_button_index++)
	{
		int16 keyboard_button_index = static_cast<uns16>(preferences->control_to_keyboard_mapping[control_button_index]);
		uns8 primary_keyboard_button_index = (uns8)(((uns16)keyboard_button_index >> 0) & UINT8_MAX);
		uns8 secondary_keyboard_button_index = (uns8)(((uns16)keyboard_button_index >> 8) & UINT8_MAX);

		c_console::write_line("control_to_keyboard_mapping[%s] = { %d, %s, %s, %s }", button_names[control_button_index],
			keyboard_button_index,
			VALID_INDEX(keyboard_button_index, k_key_code_count) ? key_names[keyboard_button_index] : "<invalid>",
			VALID_INDEX(primary_keyboard_button_index, k_key_code_count) ? key_names[primary_keyboard_button_index] : "<invalid>",
			VALID_INDEX(secondary_keyboard_button_index, k_key_code_count) ? key_names[secondary_keyboard_button_index] : "<invalid>");
	}
