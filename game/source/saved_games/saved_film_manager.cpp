...
void saved_film_manager_get_director_state(s_saved_film_manager_director_state* director_state_out)
{
	...
	for (int32 user_index = 0; user_index < 4; user_index++)
	{
		...
		{
			c_saved_film_director* saved_film_director = (c_saved_film_director*)director_get(user_index);
			int32 player_index = saved_film_director->get_watched_player();

			s_saved_film_manager_user_director_state* user_director_state =
				&director_state_out->user_director_states[user_index];

			user_director_state->observer_result = *observer_get_camera(user_index);
			user_director_state->camera_mode = saved_film_director->get_camera()->get_type();

			// $TODO c_observer_director::get_desired_camera_mode()
			user_director_state->desired_camera_mode = saved_film_director->m_desired_camera_mode;

			user_director_state->valid = true;
			user_director_state->camera_target_player_absolute_index = player_index;
		}
	}
}
...
bool saved_film_manager_get_last_recorded_film(char* filepath, int32 maximum_characters, s_saved_game_item_metadata* out_optional_metadata)
{
	...
	{
		if (first_iteration || file_compare_last_modification_dates(&mod_date, &newest_file_mod_date) > 0)
		{
	...
	if (file_found)
	{
		...
		{
			file_found = saved_game_read_metadata_from_file(&newest_file, out_optional_metadata);
		}
	}
	return file_found;
}
...
{
	e_saved_film_snippet_state snippet_state = _saved_film_snippet_state_none;
	if (saved_film_manager_snippets_available())
	{
		snippet_state = saved_film_snippet_get_current_state();
	}
	return snippet_state;
}
...
int32 saved_film_manager_get_ticks_remaining()
{
	...
	int32 ticks_remaining = 0;
	if (saved_film_manager_globals.saved_film.get_film_state() == _saved_film_open_for_read)
	{
		ticks_remaining = saved_film_manager_globals.saved_film.get_ticks_remaining();
	}
...
bool saved_film_manager_handle_revert(int32 saved_film_file_position, int32 film_tick)
{
	...
	if (game_is_authoritative_playback())
	{
		success = saved_film_manager_globals.saved_film.handle_revert(saved_film_file_position, film_tick);
	}
	return success;
}
...
{
	//bool pending_global_state_change;
	...
	if (!game_is_ui_shell() && !main_game_reset_in_progress())
	{
		...
		if (game_is_playback())
		{
			if (game_is_authoritative_playback())
			{
				...
				{
					int32 film_signature_size = saved_film_manager_globals.saved_film.get_map_signature_size();
					const byte* film_signature_bytes = saved_film_manager_globals.saved_film.get_map_signature_bytes();

					bool use_full_language_dependent_signature = !game_options_valid() || !game_is_campaign();

					if (cache_file_content_signatures_match(local_signature_size, local_signature_bytes, film_signature_size, film_signature_bytes, use_full_language_dependent_signature))
					{
					...
					else
					{
						event(_event_error, "networking:saved_film:manager: signature MISMATCH (%s), film signature (%s) does not match our local one (%s)",
							use_full_language_dependent_signature ? "full language dependent" : "language neutral only",
							cache_file_signature_summary(film_signature_size, film_signature_bytes).get_string(),
							cache_file_signature_summary(local_signature_size, local_signature_bytes).get_string());
...
void saved_film_manager_load_pending_gamestate()
{
	...
	//	event(...);
	//	{
	//		int32 original_file_position;
	//		event(...);
	//		{
	//			int32 update_number;
	//			s_saved_film_update saved_film_update;
	//			event(...);
	//			event(...);
	//			{
	//				int32 compressed_game_state_size;
	//				int32 buffer_size;
	//				byte* compressed_game_state;
	//				event(...);
	//			}
	//		}
	//		event(...);
...
void saved_film_manager_notify_remote_end_film()
{
	...
	else if (game_is_authoritative_playback())
	{
...
bool saved_film_manager_open_film_for_writing(const char* film_name, const game_options* options)
{
	...
	else
	{
		...
		success = saved_film_manager_globals.saved_film.open_for_write(saved_film_path.get_string(), options, controller_get_first_non_guest_signed_in_controller());
		if (success)
		{
			saved_film_manager_globals.saved_film_name.set(film_name);
		}
...
void saved_film_manager_preview_snippet_start()
{
	...
	else if (snippet_state != _saved_film_snippet_state_recorded_and_ready)
	{
		event(_event_warning, "networking:saved_film:manager: can't preview snippet [current in state %d]",
			snippet_state);
	}
	else if (!saved_film_snippet_preview_start())
	{
...
void saved_film_manager_preview_snippet_stop()
{
	...
	else if (snippet_state != _saved_film_snippet_state_previewing)
	{
		event(_event_warning, "networking:saved_film:manager: can't stop snippet preview [current in state %d]",
			snippet_state);
	}
	else if (saved_film_manager_get_current_tick_estimate() <= saved_film_manager_get_snippet_start_tick())
	{
	...
	else if (!saved_film_snippet_preview_stop())
	{
...
void saved_film_manager_replay_film()
{
	...
	else if (!saved_film_history_ready_for_revert_or_reset())
	{
...
void saved_film_manager_request_revert(e_saved_film_revert_type desired_revert_type)
{
	...
	else if (!saved_film_history_can_revert_by_type(desired_revert_type))
	{
...
bool saved_film_manager_rewind_and_seek_to_film_tick(int32 film_tick, bool seek_and_stop)
{
	...
	else if (saved_film_manager_has_pending_global_state_change())
	{
...
bool saved_film_manager_seeking(int32* seek_time_available_out)
{
	...
	{
		int32 current_tick = saved_film_manager_get_current_tick_estimate();
		if (saved_film_manager_globals.seek_film_tick >= current_tick)
		{
			*seek_time_available_out = saved_film_manager_globals.seek_film_tick - current_tick;
			seeking = true;
		}
	}
	return seeking;
...
bool saved_film_manager_should_record_film(const game_options* options)
{
	...
	bool record_film = saved_film_manager_should_record_film_default;
	if (options->game_mode == _game_mode_campaign)
	{
		...
		{
			record_film = level_data.flags.test(_level_allows_saved_films);
		}
	}
...
void saved_film_manager_start_recording_snippet()
{
	...
	else
	{
		...
		else if (saved_film_manager_get_snippet_start_tick() != NONE)
		{
		...
		else if (controller_get_first_non_guest_signed_in_controller() == k_no_controller)
		{
		...
		else if (!saved_film_snippet_start_recording())
		{
...
void saved_film_manager_stop_recording_snippet()
{
	...
	else
	{
		...
		else if (!saved_film_snippet_stop_recording())
		{
...
int32 saved_film_manager_upload_start(int32 maximum_file_count, s_file_reference* out_file_list)
{
	...
	//int32 file_count;
	//{
	//	c_static_string<128> source_file_path;
	//	bool file_name_found;
	//	c_static_string<128> destination_file_path;
	//	{
	//		int32 content_item_index;
	//		c_content_catalogue* content_catalogue;
	//		{
	//			c_debug_output_path debug_output_path;
	...
	//	}
	//	{
	//		s_file_reference temp_reference;
	//		{
	//			char file_name[256];
	//			c_debug_output_path debug_output_path;
	//		}
	//	}
	//	{
	//		e_saved_film_state current_film_state;
	//		e_saved_film_file_path_creation_purpose name_build_creation_purpose;
	//		c_debug_output_path debug_output_path;
	//	}
	//}
}
...
bool saved_film_manager_write_simulation_update(const s_simulation_update* update)
{
	...
	else if (!saved_film_manager_globals.saved_film.write_simulation_update(update))
	{
