...
public:
	...
	bool decode_packet(long data_buffer_size, uint8 const* data_buffer, s_link_packet* packet) const;
	void destroy_endpoints();
	void destroy_link();
	void encode_packet(s_link_packet const* packet, long* data_length, uint8* data_buffer, long data_buffer_size) const;
	...
	bool read_data_immediate(transport_address* address, long* packet_data_length, uint8* packet_buffer, long packet_buffer_size);
	bool read_packet_internal(s_link_packet* packet);
	void send_broadcast(c_bitstream const* game_data, long* out_size_on_wire);
	void send_channel(c_bitstream const* packet, long voice_data_length, uint8 const* voice_data, transport_address const* remote_address, long* out_size_on_wire);
