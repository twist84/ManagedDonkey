...
bool c_network_link::adjust_packet_size(bool game_data, long voice_data_length, long* game_data_length) const
{
	...
	else
	{
		...
		if (payload_length < 0)
		{
			WARNING_EVENT("networking:link: requested in-channel VDP packet (%d bytes voice, %d bytes data) exceeds maximum payload (%d bytes), impossible to send", voice_data_length, *game_data_length, maximum_payload);
			return false;
		}
		else
		{
...
bool c_network_link::create_endpoint(e_transport_type type, word port, bool set_broadcast_option, transport_endpoint** out_endpoint)
{
	...
	//if (!endpoint)
	//{
	//	ERROR_EVENT("networking:link: create_endpoint[%d] - unable to create endpoint!", type);
	//	return false;
	//}
	//
	...
	//{
	//	ERROR_EVENT("networking:link: create_endpoint(%d) - unable to setup endpoint!", type);
...
bool c_network_link::initialize_link()
{
	...
	//	ERROR_EVENT("networking:link: initialize() failed to create packet endpoints!");
	//if (result)
	//	m_initialized = true;
	//else
	//	destroy_link();
