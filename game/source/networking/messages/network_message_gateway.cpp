...
bool c_network_message_gateway::receive_out_of_band_packet(transport_address const* incoming_address, c_bitstream* packet)
{
	...
	if (read_packet_header(packet))
	{
		...
		while (packet->read_bool("has_message"))
		{
			...
			if (!result)
			{
				VERBOSE_EVENT("networking:messages:gateway:anomaly: failed to parse %d/%s %d-byte message from '%s'",
					message_type,
			...
			VERBOSE_EVENT("networking:messages:gateway:receive: received %d/%s %d-byte message out-of-band from '%s'",
				message_type,
				m_message_types->get_message_type_name(message_type),
				message_storage_size,
				transport_address_get_string(incoming_address));
	...
	else
	{
		...
		VERBOSE_EVENT("networking:messages:gateway:anomaly: foreign packet (%d bytes %s%s) from '%s'",
			data_length,
...
void c_network_message_gateway::send_all_pending_messages()
{
	...
	if (m_outgoing_packet_pending)
	{
		...
		if (m_outgoing_packet.begin_consistency_check())
		{
			...
			if (!result)
				CRITICAL_EVENT("networking:messages:gateway:send_all_pending_messages: an outgoing message header is corrupt!");
			...
			while (m_outgoing_packet.read_bool("has_message"))
			{
				...
				if (!result)
				{
					CRITICAL_EVENT("networking:messages:gateway:send_all_pending_messages: an outgoing message payload is corrupt!");
					break;
				}
...
bool c_network_message_gateway::send_message_directed(transport_address const* outgoing_address, e_network_message_type message_type, long data_size, void const* data)
{
	...
	while (true)
	{
		...
		if (!m_outgoing_packet_pending)
		{
			...
			{
				ERROR_EVENT("networking:messages:gateway:send_message: unable to get packet size to send [tried %d bytes]", packet_size);
				break;
			}
	...
	CRITICAL_EVENT("networking:messages:gateway:send_message: message type '%s' size raw-bytes/enc-bits %d/%d doesn't fit in a packet and cannot be sent out-of-band",
		m_message_types->get_message_type_name(message_type),
		data_size,
		encoded_bits);
