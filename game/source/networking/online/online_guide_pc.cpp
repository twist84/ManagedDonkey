...
)
{
	...
	else
	{
		if (!m_instance->busy())
		{
			...
			GENERATE_EVENT(_event_warning, "ui: someone tried to get a duplicate instance of the virtual keyboard!");
	}
...
void __cdecl online_guide_show_file_share_recommendation(e_controller_index controller_index, qword user_xuid, long a3, char const* a4, s_service_record_identity const* identity, wchar_t const* a6)
{
	...
	//	GENERATE_EVENT(_event_error, "networking:online:guide: online_guide_show_file_share_recommendation called when task is already in progress");
}
...
dword __cdecl online_guide_show_friend_request_ui(e_controller_index controller_index, qword user_xuid)
{
	...
	//
	//if (FAILED(result))
	//	GENERATE_EVENT(_event_warning, "networking:online:guide: XShowFriendRequestUI call failed with result %s", online_error_get_string(error_string, result));
	//
	//return result;
...
dword __cdecl online_guide_show_friends_ui(e_controller_index controller_index)
{
	...
	//
	//if (FAILED(result))
	//	GENERATE_EVENT(_event_error, "networking:online:guide: XShowFriendsUI() returned #%ld for controller #%ld", result, controller_index);
	//
	//return result;
...
dword __cdecl online_guide_show_gamer_card_ui(e_controller_index controller_index, qword user_xuid)
{
	...
	//
	//if (FAILED(result))
	//	GENERATE_EVENT(_event_warning, "networking:online:guide: XShowGamerCardUI call failed with result %s", online_error_get_string(error_string, result));
	//
	//return result;
...
dword __cdecl online_guide_show_player_review_ui(e_controller_index controller_index, qword target_user_xuid)
{
	...
	//
	//if (FAILED(result))
	//	GENERATE_EVENT(_event_warning, "networking:online:guide: XShowPlayerReviewUI call failed with result %s", online_error_get_string(error_string, result));
	//
	//return result;
...
bool __cdecl online_guide_show_sign_in_ui(long pane_count, dword_flags flags)
{
	...
	//if (result)
	//	GENERATE_EVENT(_event_warning, "networking:online:guide: XShowSigninUI failed with result %08x.", result);
	//
	//return result == 0;
