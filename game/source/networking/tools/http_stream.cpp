...
bool c_http_post_source::set_source_as_file(s_file_reference* contents_file)
{
	...
	bool success = file_get_size(contents_file, &m_contents_length);
	if (success)
	{
...
c_http_stream::c_http_stream() :
	m_url(),
	m_http_header(),
	m_http_header_length(),
	m_extra_headers(),
	m_overall_position(0)
{
}
...
{
	m_overall_position = position;
}

int32 c_http_stream::get_position()
{
	return m_overall_position;
}
...
{
	m_http_header.print("GET %s HTTP/1.0\r\n%s\r\n", m_url.get_string(), m_extra_headers.get_string());
	m_http_header_length = m_http_header.length();
}
...
{
	//bool success = INVOKE_CLASS_MEMBER(0x00432A10, c_http_get_stream, read, buffer, buffer_max_length, bytes_read);
	...
	if (verify_nescessary_state_is_set())
	{
		...
		if (!at_end())
		{
			while (true)
			{
				...
				int32 position = m_overall_position;
				if (position >= m_http_header_length)
				{
				...
				if (dest_buffer_length > m_http_header_length - position)
				{
					dest_buffer_length = m_http_header_length - position;
				}

				csmemmove(dest_buffer, &m_http_header.get_string()[position], dest_buffer_length);

				dest_buffer += dest_buffer_length;
				m_overall_position += dest_buffer_length;
...
{
	return m_http_header_length;
}

bool c_http_get_stream::at_end()
{
	return m_overall_position == m_http_header_length;
}
...
void c_http_post_stream::build_headers()
{
	...
	if (m_http_post_source.is_json())
	{
		...
		m_http_header.print("POST %s HTTP/1.0\r\nContent-Type: application/json\r\nContent-Length: %d\r\n%s\r\n",
			m_url.get_string(),
			m_http_post_source.get_contents_length() - m_http_post_source.get_start_position() + m_mime_footer_length + 2,
	...
	else
	{
		...
		m_mime_header.print("--BUNGIEr0x0rz\r\nContent-Disposition: form-data; name=\"upload\"; filename=\"%s\"\r\nContent-Type: %s\r\n\r\n",
			m_http_post_source.get_filename(),
			m_http_post_source.get_content_type());
		m_mime_header_length = m_mime_header.length();

		m_http_header.print("POST %s HTTP/1.0\r\nContent-type: multipart/form-data; boundary=BUNGIEr0x0rz\r\nContent-Length: %d\r\n%s\r\n",
			m_url.get_string(),
			m_mime_header_length + m_mime_footer_length + m_http_post_source.get_contents_length() - m_http_post_source.get_start_position(),
			m_extra_headers.get_string());
		m_http_header_length = m_http_header.length();
	}
}
...
bool c_http_post_stream::verify_nescessary_state_is_set()
{
	...
	//uns8 necessary_state_is_set = m_http_post_source.get_source_type() == c_http_post_source::_contents_source_as_file
	//	|| m_http_post_source.get_source_type() == c_http_post_source::_contents_source_as_buffer;
	//return necessary_state_is_set & (m_url.length() != 0);
}

bool c_http_post_stream::read(char* buffer, int32 buffer_max_length, int32* bytes_read)
{
	return INVOKE_CLASS_MEMBER(0x00432AD0, c_http_post_stream, read, buffer, buffer_max_length, bytes_read);
	...
	if (verify_nescessary_state_is_set())
	{
		...
		while (!at_end() && len > 0)
		{
			...
			if (position >= m_http_header_length)
			{
				position -= m_http_header_length;
	
				if (position >= m_mime_header_length)
				{
					position -= m_mime_header_length;
	
					if (position >= m_http_post_source.get_contents_length())
					{
						position -= m_http_post_source.get_contents_length();
					...
					{
						if (len > m_http_post_source.get_contents_length() - position)
						{
							len = m_http_post_source.get_contents_length() - position;
						}
	
						switch (m_http_post_source.get_source_type())
						{
						case c_http_post_source::_contents_source_as_buffer:
						{
							csmemcpy(buf, &m_http_post_source.get_contents_buffer()[m_http_post_source.get_start_position()], len);
						}
						break;
						case c_http_post_source::_contents_source_as_file:
						{
							position += m_http_post_source.get_start_position();
							if (!file_read_from_position(m_http_post_source.get_contents_file(), position, len, false, buf))
							{
								success = false;
							}
						}
			...
			{
				if (len > m_http_header_length - position)
				{
					len = m_http_header_length - position;
				}
				csmemcpy(buf, &m_http_header.get_string()[position], len);
			}
	
			buf += len;
			m_overall_position += len;
			len = buffer_max_length - (buf - buffer);
		}
...
int32 c_http_post_stream::get_length()
{
	...
	return m_http_header_length
		+ m_mime_header_length
		+ m_mime_footer_length
		+ m_http_post_source.get_contents_length() - m_http_post_source.get_start_position();
}
...
bool c_http_post_stream::at_end()
{
	...
	return m_overall_position == get_length();
}
