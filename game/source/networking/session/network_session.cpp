...
bool c_network_session::handle_leave_internal(long peer_index)
{
	...
	//	//else
	//	//{
	//	//	WARNING_EVENT("networking:session:membership: [%s] leave-request from peer [%s] denied, session membership is locked (state %s)",
	...
	//else
	//{
	//	MESSAGE_EVENT("networking:session:membership: %s leave-request accepted for peer [%s]",
	//		managed_session_get_id_string(m_managed_session_index),
	//		get_peer_description(peer_index));
	//
...
bool c_network_session::host_assume_leadership()
{
	...
	//		{
	//			STATUS_EVENT("networking:session:membership: [%s] assuming leadership ([#%d] -> [#%d])",
	//				managed_session_get_id_string(m_managed_session_index),
	...
	//	else
	//	{
	//		ERROR_EVENT("networking:session:membership: [%s] failed assuming leadership, not host of session",
	//			managed_session_get_id_string(m_managed_session_index));
	//	}
	//}
	//else
	//{
	//	ERROR_EVENT("networking:session:membership: [%s] failed assuming leadership, session not established",
	//		managed_session_get_id_string(m_managed_session_index));
	//}
	//
	//return false;
...
bool c_network_session::host_boot_machine(long peer_index, e_network_session_boot_reason boot_reason)
{
	...
	//if (is_host())
	//{
	//	MESSAGE_EVENT("networking:session:membership: [%s] booting machine [#%d] locally [reason %d]",
	//		managed_session_get_id_string(m_managed_session_index),
	//		peer_index,
	//		boot_reason);
	//
	...
	//	{
	//		MESSAGE_EVENT("networking:session:membership: we are the host and are being booted, leaving...");
	//
...
void c_network_session::host_connection_refused(transport_address const* address, e_network_join_refuse_reason refuse_reason)
{
	...
	//	{
	//		MESSAGE_EVENT("networking:session:join: [%s] received join-queue ping from host, waiting",
	//			managed_session_get_id_string(m_managed_session_index));
	//
	...
	//	else
	//	{
	//		MESSAGE_EVENT("networking:session:join: [%s] host connection refused, aborting join",
	//			managed_session_get_id_string(m_managed_session_index));
	//
...
bool c_network_session::host_set_player_current_properties(long player_index, struct s_player_configuration const* player_data)
{
	...
	//	{
	//		STATUS_EVENT("networking:session:membership: [%s] local host updating current player properties for player [#%d]",
	//			managed_session_get_id_string(m_managed_session_index),
	//			player_index);
	//
	//		return true;
	...
	//	else
	//	{
	//		STATUS_EVENT("networking:session:membership: [%s] local host updating player properties for invalid player [#%d]",
	//			managed_session_get_id_string(m_managed_session_index),
	//			player_index);
	//	}
	//}
...
bool c_network_session::peer_request_player_desired_properties_update(long player_update_number, e_controller_index controller_index, s_player_configuration_from_client const* player_data_from_client, dword player_voice)
{
	...
	if (is_host())
	{
		...
		{
			ERROR_EVENT("networking:session:membership: [%s] local host requested player-properties does not exist",
				managed_session_get_id_string(m_managed_session_index));
		...
		STATUS_EVENT("networking:session:membership: [%s] local host applying player-properties for player [#%d]",
			managed_session_get_id_string(m_managed_session_index),
			player_index);
	...
	{
		STATUS_EVENT("networking:session:membership: [%s] sending player-properties request",
			managed_session_get_id_string(m_managed_session_index));
...
bool c_network_session::handle_parameters_update(s_network_message_parameters_update const* message)
{
	...
	//if (is_host())
	//{
	//	ERROR_EVENT("networking:session:parameters: [%s] parameters-update received and we are the host",
	//		managed_session_get_id_string(m_managed_session_index));
	//
	//	return false;
	//}
	...
	//		return true;
	//
	//	ERROR_EVENT("networking:session:parameters: [%s] failed to handle parameters-update, we must disconnect [%s]",
	//		managed_session_get_id_string(m_managed_session_index),
	//		get_state_string());
	//
	...
	//if (leaving_session())
	//{
	//	MESSAGE_EVENT("networking:session:parameters: [%s] parameters-update received but we're leaving [%s], ignoring the update",
	//		managed_session_get_id_string(m_managed_session_index),
	//		get_state_string());
	//
	//	return false;
	//}
	//
	//MESSAGE_EVENT("networking:session:parameters: [%s] parameters-update received and we are in a bad state to handle the update [%s]",
	//	managed_session_get_id_string(m_managed_session_index),
	//	get_state_string());
	//
	//return false;
...
bool c_network_session::handle_player_properties(c_network_channel* channel, s_network_message_player_properties const* message)
{
	...
	if (established() && is_host())
	{
		...
		{
			MESSAGE_EVENT("networking:session:membership: [%s] player-properties received from invalid peer [#%d]",
				managed_session_get_id_string(m_managed_session_index),
				peer_index);
		}
		//else if (message->user_index < 0 || message->user_index >= 4)
		//{
		//	ERROR_EVENT("networking:session:membership: [%s] player-properties received for invalid user_index [#%d]",
		//		managed_session_get_id_string(m_managed_session_index),
		//		message->user_index);
		//}
		else
		{
			...
			if (player_index == NONE)
			{
				WARNING_EVENT("networking:session:membership: [%s] player-properties received but no player associated with peer [#%d]",
					managed_session_get_id_string(m_managed_session_index),
					peer_index);
			}
			else
			{
				...
				STATUS_EVENT("networking:session:membership: [%s] player-properties accepted for peer/player [#%d]/[#%d]",
					managed_session_get_id_string(m_managed_session_index),
					peer_index,
					peer_index);
			}
	...
	{
		WARNING_EVENT("networking:session:membership: [%s] player-properties received but not host, can't update players",
			managed_session_get_id_string(m_managed_session_index));
	}
