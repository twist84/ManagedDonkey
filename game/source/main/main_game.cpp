...
bool __cdecl main_game_change_immediate(game_options const* options)
{
	...
	if (options)
	{
		...
		if (main_game_load_map(options) && (main_render_assert_no_pending_messages(), main_game_start(options)))
		{
			...
			MESSAGE_EVENT("lifecycle: MAP-LOADED %s", options->scenario_path.get_string());
			...
			case _game_mode_multiplayer:
			{
				...
				MESSAGE_EVENT("lifecycle: MULTIPLAYER-GAME %s", game_engine_name);
				MESSAGE_EVENT("lifecycle: MULTIPLAYER-VARIANT %s", game_variant_name);
			}
			break;
			}
		...
		else
		{
			...
			CRITICAL_EVENT("main_game_change_immediate() failed for '%s', cannot load game", options->scenario_path.get_string());
		...
		else
		{
			...
			if (game_in_progress() && game_is_campaign() && !game_is_survival())
			{
				...
				case _game_progression_level_none:
					...
					//	break;
					//default:
					//	ERROR_EVENT("networking:main_game: invalid map advance type %d!", map_advance_type);
					//	break;
				}
			}
			...
			if (!goto_next_level)
			{
				...
				{
					MESSAGE_EVENT("networking:main_game: congratulations, you won the game! or, the next map failed to load");

					if (game_in_progress() && game_is_ui_shell())
					{
						MESSAGE_EVENT("networking:main_game: already in the ui, not ending the simulation");
					}
					else
					{
...
void __cdecl main_game_load_panic()
{
	...
	if (debug_load_panic_to_main_menu)
	{
		...
		{
			CRITICAL_EVENT("main_game_load_panic: recursion lock triggered (we must have failed to load the main menu from a panic state)");
		}
		else
		{
			game_options options{};
	...
	{
		CRITICAL_EVENT("main game load failed, unable to recover, aborting to pregame");
...
void __cdecl main_game_load_from_core_name(char const* core_name)
{
	...
	{
		WARNING_EVENT("Failed to get game options from core (so I can't load it!)");
	}
}
...
void __cdecl main_game_reset_map(bool reset_map_random)
{
	...
	if (!success)
	{
		CRITICAL_EVENT("main_game_reset_map couldn't restart the game!");
		main_game_load_panic();
	}
}
