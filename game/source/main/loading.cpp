...
void __cdecl loading_basic_progress_update_fraction(real32 progress_fraction)
{
	...
	//	loading_basic_progress_update_size(uns32((1.0f - progress_fraction) * loading_globals.basic_progress_maximum_bytes[loading_globals.basic_progress_phase]));
	//}
}
...
e_main_pregame_frame __cdecl main_loading_get_loading_status(c_static_wchar_string<12288>* loading_progress_string)
{
	...
	{
		pregame_frame = _main_pregame_frame_normal;
	}
	else if (!disable_progress_screen)
	{
		if (loading_globals.tag_load_in_progress || loading_globals.progress_valid || loading_globals.spinner_enabled)
		{
			if (!game_in_progress() || loading_globals.tag_load_in_progress)
			{
				static const wchar_t* spinner_chars[] = { L"/", L"-", L"\\" };
				static int32 spinner_index = 8 * system_milliseconds() / 1000 % NUMBEROF(spinner_chars);
				...
				if (loading_globals.tag_load_in_progress)
				{
					...
					{
						last_time = time;
						spinner_index = (spinner_index + 1) % NUMBEROF(spinner_chars);
					}
				}
				...
				if (!game_in_progress() || loading_globals.tag_sync_in_progress || loading_globals.tag_load_in_progress)
				{
					...
					if (string_is_not_empty(loading_globals.blocking_map_name))
					{
						if (loading_progress_string)
						{
							...
							loading_progress_string->append_print(L"loading scenario %S...", loading_globals.blocking_map_name);
							//else
							//{
							//	loading_progress_string->append_print(L"loading ZONE SET #%d %S...", loading_globals.zone_set_index, loading_globals.blocking_map_name);
							//}
					...
					if (loading_globals.progress_valid && !loading_globals.loading_progress.is_empty())
					{
						if (loading_progress_string)
						{
					...
					else if (loading_progress_string)
					{
						loading_progress_string->append_print(L"|n");
					}
					...
					pregame_frame = _main_pregame_frame_loading_debug;
				}
			}
		}
