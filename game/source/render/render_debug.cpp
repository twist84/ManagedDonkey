...
long __cdecl render_debug_add_cache_string(char const* string)
{
	...
	{
		WARNING_EVENT("render debug cache string overflow");
...
void __cdecl render_debug_add_cache_entry(short type, ...)
{
	...
	{
		WARNING_EVENT("render debug cache overflow.");
...
void __cdecl render_debug_cache_draw(bool render_game_tick_cache, bool only_render_strings, bool clear_cache)
{
	...
	if (g_render_debug_globals->cache_count - cache_start_index > 0)
	{
		...
		for (short cache_index = cache_start_index; cache_index < g_render_debug_globals->cache_count; cache_index++)
		{
			...
			{
				WARNING_EVENT("render:debug: unknown debug render cache layer %d!!!", cache_layer);
			}
...
void c_render_debug_line_drawer::flush()
{
	...
	{
		ERROR_EVENT("can't use debug_line_drawer w/o immediate debug drawing");
	}
